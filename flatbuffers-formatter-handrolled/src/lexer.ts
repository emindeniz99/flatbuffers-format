...
// Lex-time cap on consecutive blank_line markers. The printer's
// `maxBlankLines` option does the actual user-visible capping; this
// just keeps memory bounded on pathological inputs.
const LEX_BLANK_CAP = 32;

function countTailingBlanks(pending: Trivia[]): number {
  let n = 0;
  for (let i = pending.length - 1; i >= 0; i--) {
    if (pending[i]!.kind === "blank_line") n++;
    else break;
  }
  return n;
}
...
export function tokenize(source: string): Token[] {
  ...
  const readNumber = (): Token => {
    ...
    const startCol = col;
    const start = i;
    // Hex prefix — could be either a hex int (`0xFF`) or a C99-style hex
    // float (`0x1.8p3`, `0x1p-3`, `0x.8p4`). Hex floats are distinguished
    // by the mandatory binary-exponent part (`[pP][+-]?[0-9]+`); without
    // an exponent it's a hex int.
    if (peek() === "0" && (peek(1) === "x" || peek(1) === "X")) {
      advance(2);
      const intDigitsStart = i;
      while (isHex(peek())) advance();
      const haveIntDigits = i > intDigitsStart;
      // Look ahead for the optional `.[hex]*` then mandatory `pP`.
      // Lookahead only — we don't commit unless the full hex-float
      // pattern matches, so a bare `0x1.` stays as an int + dot.
      let look = 0;
      let haveFracDigits = false;
      if (peek(look) === ".") {
        const fracStart = look + 1;
        look++;
        while (isHex(peek(look))) look++;
        haveFracDigits = look > fracStart;
      }
      const expChar = peek(look);
      if ((haveIntDigits || haveFracDigits) && (expChar === "p" || expChar === "P")) {
        for (let k = 0; k < look; k++) advance(); // consume `.[hex]*`
        advance(); // p / P
        if (peek() === "+" || peek() === "-") advance();
        while (isDigit(peek())) advance();
        return makeToken("float", source.slice(start, i), startLine, startCol);
      }
  ...
  while (!eof()) {
    ...
    if (c === "\n") {
      ...
      // Emit one blank_line marker per source blank line up to a
      // safety cap. The printer's `maxBlankLines` option caps further;
      // this cap just prevents a pathological file (thousands of blank
      // lines) from blowing memory.
      if (consecutiveNewlines >= 2) {
        const blanksSoFar = countTailingBlanks(pending);
        if (blanksSoFar < LEX_BLANK_CAP) {
