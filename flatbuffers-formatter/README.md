# flatbuffers-formatter-antlr

A FlatBuffers (`.fbs`) schema formatter built on an
**ANTLR4-generated parser**. Sibling to
[`../flatbuffers-formatter`](../flatbuffers-formatter), which does the
same job with a hand-rolled lexer + recursive-descent parser. Same
input, byte-identical output — different implementation strategy.
...
- A Node CLI (`fbs-fmt-antlr`) wraps the same API; the core has no
  Node imports so it also runs in the browser via `antlr4ng`.
...
# fix files in place
node dist/src/cli.js --write examples/

# check formatting in CI
node dist/src/cli.js --check examples/

# from stdin
cat schema.fbs | node dist/src/cli.js -

# alternate name when installed
fbs-fmt-antlr fix path/to/schemas/
```
